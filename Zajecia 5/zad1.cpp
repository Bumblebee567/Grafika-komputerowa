...
std::vector<GLfloat> vertices(3 * (N + 2));
std::vector<int> triangles((N + 1) * 6);
...
void CreateObject()
{
	...
	{
		const GLfloat x = 0.5f * std::sin(2.0f * GL_PI * ((GLfloat)i / N));
		const GLfloat y = 0.5f * std::cos(2.0f * GL_PI * ((GLfloat)i / N));
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("Swiatlo i teksturowanie");
